...
async def add_node(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    updated = await increment_set(NODES, context.args, context)

    if updated:
        try:
            data.update_nodes(NODES)
...
async def del_node(updated: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    updated = await reduce_set(NODES, context.args, context)
...
async def increment_set(watchlist, nodes, context):
    incremented = False
    for node in nodes:
...
async def reduce_set(watchlist, nodes, context):
    reduced = False
    for node in nodes:
        try:
